...
def run_stream_server(opt):
    ...
    main_cap = main_cv.VideoCapture(0)
    main_cap.set(main_cv.CAP_PROP_BUFFERSIZE, 3)
    main_cap.set(main_cv.CAP_PROP_FPS, 30)  # TODO: 체크할것
    if opt.vid_res == "adaptive":
        if opt.down != 1:
            print("[Alert] opt[--vid-res] is \'adaptive\', so opt[down] is disabled.")
    else:
        main_cap.set(main_cv.CAP_PROP_FRAME_WIDTH, AVAILABLE_RESOLUTIONS[opt.vid_res][1])
        main_cap.set(main_cv.CAP_PROP_FRAME_HEIGHT, AVAILABLE_RESOLUTIONS[opt.vid_res][0])
    ...
    while True:
        ...
        if ret:
            ...
            if opt.down != 1:
                img_det = main_cv.resize(img_raw, [i // opt.down for i in AVAILABLE_RESOLUTIONS[opt.vid_res][:2][::-1]])
            else:
